...
import { getLabOrdersColumns } from './lab-orders-columns'
import type { LabOrder } from '../types'
import type { LabOrdersSearch } from '../types'
...
const STATUS_FILTER_OPTIONS = [
  { label: 'Chờ thực hiện', value: 'CHO_THUC_HIEN' },
  { label: 'Đang thực hiện', value: 'DANG_THUC_HIEN' },
  { label: 'Chờ kết quả', value: 'CHO_KET_QUA' },
...
export function LabOrdersTable({
  ...
  const columns = useMemo(
    () => getLabOrdersColumns({ onViewDetail, onProcessLabOrder }),
    [onViewDetail, onProcessLabOrder]
  )
    ...
    <div className='flex flex-1 flex-col gap-4'>
      <div className='flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between'>
        <div className='flex flex-1 flex-col gap-4'>
          <DataTableToolbar
            table={table}
            searchPlaceholder='Tìm theo mã phiếu, tên xét nghiệm...'
            filters={[
              {
                columnId: 'status',
                title: 'Trạng thái',
      ...
      <div className='flex flex-col gap-2 rounded-lg bg-card/40 p-4'>
        <div className='flex flex-wrap items-center justify-between gap-2 text-sm text-muted-foreground'>
          <span>Tổng cộng {total} phiếu xét nghiệm</span>
        </div>
              ...
              {isLoading && data.length === 0 ? (
                ...
                <TableRow>
                  <TableCell colSpan={table.getAllColumns().length} className='py-10 text-center text-muted-foreground'>
                    Không có phiếu xét nghiệm phù hợp.
                  </TableCell>
                </TableRow>
              )}
            </TableBody>
        ...
        <div className='flex flex-col gap-3 border-t pt-4 sm:flex-row sm:items-center sm:justify-between'>
          <div className='text-sm text-muted-foreground'>
            Hiển thị {startRow} – {endRow} trên tổng {total} phiếu xét nghiệm.
          </div>
